...
if (
    ...
    if (count($result) == 2) {
        if ($result['estado'] == 1 && ($result['id_rol'] == 1 || $result['id_rol'] == 1)) {
        ...
        if ($result[0] == 1){
            echo '{"data":"Este cliente ya existe en la base de datos","response":"error"}';
            exit;
        }
        
        $stmt = $mysql -> consulta("SELECT COUNT(correo) FROM cliente where correo = ?",[$email]);
        $result = $stmt->fetch(PDO::FETCH_NUM);
        if ($result[0] == 1){
            echo '{"data":"El correo ya existe","response":"error"}';
            exit;
        }       
        $mysql -> consulta("INSERT INTO cliente VALUES(?,?,?,?,?,?,?)",[$id,$names,$lastname,$address,$phone,$email,1]);
        echo '{"data":"Cliente creado con éxito","response":"success"}';
        exit;
    }
    }
    else{
